...
namespace All_in_One.Services.CalculateService.Functions
{
    internal class GetDataFromTextFileLogs
    {
        ...
        /// <returns></returns>
        public List<PlayerExtractedData> GetPlayerDataFromTextFileLogs(string path)
        {
                    ...
                    Consumables.AcceptedConsumables.ForEach(consumable =>
                    {
                        if (line.Contains(consumable))
                        {
                            ...
                            if (!result.Exists(entry => entry.PlayerName == player))
                            {
                                if (isFlask)
                                {
                                    result.Add(new PlayerExtractedData() { PlayerName = player, Consumable1 = line.Split(',')[10], Consumable2 = line.Split(',')[10] });
                                }
                                else
                                {
                                    result.Add(new PlayerExtractedData() { PlayerName = player, Consumable1 = line.Split(',')[10] });
                                }
                            }
                            else
                            {
                                ...
                                if (!PlayerDKPEntry.Consumable1.Contains(consumable) && PlayerDKPEntry.Consumable2 == "")
                                {
                                    PlayerDKPEntry.Consumable2 = line.Split(',')[10];
                                }
